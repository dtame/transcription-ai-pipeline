...
def run_project_pipeline(project: AudioProject, model=None) -> dict:
    ...
    def step_chunks():
        from app.chunk_service import is_chunk_stale, reset_stale_processed_files
        ...
        for chunk_path in chunk_paths:
            ...
            if already_done and is_chunk_stale(chunk_path):
                # Chunk was generated by old code — force reprocessing
                from app.project_state import reset_chunk_for_reprocessing
                reset_chunk_for_reprocessing(state, chunk_name)
                stale_chunks.append(chunk_name)
                print(
                    f"[pipeline] Chunk obsolète (ancien code) réinitialisé "
                    f"pour retraitement : {chunk_name}"
                )
            else:
                register_chunk(state, chunk_name)

        if stale_chunks:
            reset_stale_processed_files(
                project.name,
                stale_chunks,
                project.output_dir,
            )
            print(
                f"[pipeline] {len(stale_chunks)} chunk(s) obsolète(s) réinitialisé(s)."
            )
...
def run_full_rebuild_pipeline(project: AudioProject) -> dict:
    ...
    from app.chunk_service import reset_stale_processed_files
    from app.paths import SORTIE_DIR
    ...
    def step_chunks():
        chunk_paths = create_project_chunks(project)
        state = load_project_state(project)
        # After full reset, all chunks are already "pending" — just register new ones
        for chunk_path in chunk_paths:
